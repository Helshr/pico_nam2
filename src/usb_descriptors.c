...
char const *string_desc_arr[] = {
    ...
    "Output",                    // 4: Audio Interface (STRID_AUDIO_OUT)
    "Input",                     // 5: Audio Interface (STRID_AUDIO_IN)
};
...
size_t usb_audio_frame_num(size_t frame_size) { return frame_size / CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX / CFG_TUD_AUDIO_FUNC_1_FORMAT_1_N_BYTES_PER_SAMPLE_RX;; }
size_t usb_audio_frame_size(void) { return (size_t)((current_sample_rate / 1000) * CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX * CFG_TUD_AUDIO_FUNC_1_FORMAT_1_N_BYTES_PER_SAMPLE_RX); }
...
static bool tud_audio_clock_set_request(uint8_t rhport, audio20_control_request_t const *request,
                                        uint8_t const *buf) {
    (void)rhport;
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request) {
    ...
    // Let the audio loop know when the host opens/closes the mic (IN) stream so
    // it can (re)start from an empty staging ring and gate its FIFO writes.
    usb_audio_streaming_changed(itf, alt != 0);
    return true;
}
